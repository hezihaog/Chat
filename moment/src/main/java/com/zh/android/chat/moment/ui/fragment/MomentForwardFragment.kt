 ...
 * 动态转发列表
 */
class MomentForwardFragment : BaseFragment() {
    ...
    private val mListAdapter by lazy {
        MultiTypeAdapter(mListItems).apply {
            register(MomentForwardRecordModel::class.java, MomentForwardRecordViewBinder())
        }
    }
    ...
    private fun refresh() {
        ...
        getMomentForwardList(mMomentId, mCurrentPage)
    }

    private fun loadMore() {
        val nextPage = mCurrentPage + 1
        getMomentForwardList(mMomentId, nextPage)
    }

    /**
     * 获取动态转发列表
     */
    private fun getMomentForwardList(
        momentId: String,
        pageNum: Int
    ) {
        val isFirstPage = pageNum == ApiUrl.FIRST_PAGE
        val pageSize = ApiUrl.PAGE_SIZE
        mMomentPresenter.getMomentForwardList(momentId, pageNum, pageSize)
            .ioToMain()
