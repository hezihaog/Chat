...
class ImageMsgReceiverViewBinder(
    ...
    ItemViewBinder<ChatRecord, ImageMsgReceiverViewBinder.ViewHolder>() {
    override fun onCreateViewHolder(inflater: LayoutInflater, parent: ViewGroup): ViewHolder {
        return ViewHolder(
            inflater.inflate(
                R.layout.conversation_image_msg_receiver_item_view,
                parent,
                false
            )
        )
    ...
    override fun onBindViewHolder(holder: ViewHolder, item: ChatRecord) {
        item.run {
            holder.vAvatar.loadUrlImageToRound(
                ApiUrl.getFullImageUrl(toUser.picNormal)
            )
